...
shinyServer(function(input, output) {
  ...
  withProgress(message="Creando modelos", {
    ...
    omitinfinite<-omit[which(is.finite(omit$P6071)
                               & is.finite(omit$P6071S1)
                               & is.finite(omit$P6088)
                               &  is.finite(omit$P1903)
                               & is.finite(omit$P6087)),  ]
    ...
    omitinfinite<- omitinfinite[,-c(1,2,3,4,5,6)]
    daticos<-omitinfinite
    
    for(i in 1:28){
      daticos[,i]<- as.factor(omitinfinite[,i])
    }
    incProgress(25)
    ### Todos los nombres
